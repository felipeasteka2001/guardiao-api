...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (!new_filename || typeof new_filename !== "string") {
      return res.status(400).json({
        ok: false,
        error: "new_filename é obrigatório."
      });
    }
    ...
    const cleanNewFilename = new_filename.replace(/^\/+/, "").trim();

    if (cleanNewFilename.includes("/") || cleanNewFilename.includes("..")) {
      return res.status(400).json({
        ok: false,
        error: "new_filename deve ser apenas o nome do arquivo, sem path."
      });
    }

    const directory = getDirectory(sourcePath);
    const destinationPath = directory
      ? `${directory}/${cleanNewFilename}`
      : cleanNewFilename;
    ...
    const createResult = await githubCreateFile(
      destinationPath,
      sourceFile.content,
      `Renomear ${sourcePath} para ${destinationPath}: ${reason}`
    );

    await githubDeleteFile(
      sourcePath,
      sourceFile.sha,
      `Remover arquivo antigo após renomear para ${destinationPath}: ${reason}`
    );
